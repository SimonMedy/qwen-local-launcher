...
if (-not $OutputPath) { $OutputPath = Join-Path $root 'Qwen Local Launcher.exe' }
...
function New-QwenLauncherIcon {
    ...
    try {
        ...
        $bg = New-Object System.Drawing.Drawing2D.LinearGradientBrush(
            $rect,
            [System.Drawing.Color]::FromArgb(28, 30, 38),
            [System.Drawing.Color]::FromArgb(40, 44, 62),
            90.0)
        ...
        try {
            ...
            $g.DrawEllipse($ring, 58, 52, 140, 140)
            $g.DrawLine($tail, 164, 165, 209, 210)
            $g.FillEllipse($dot, 184, 47, 28, 28)
        } finally {
            $bg.Dispose(); $ring.Dispose(); $tail.Dispose(); $dot.Dispose()
        }
